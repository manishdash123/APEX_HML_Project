...
TacosNetwork::TacosNetwork(const std::shared_ptr<Topology> topology, const ChunkSize chunkSize) noexcept
    ...
    linkStartTimes = decltype(linkStartTimes)(npusCount, std::vector<Time>(npusCount, -1));
...
Time TacosNetwork::linkStartTime(const LinkId link) const noexcept {
    auto [src, dest] = link;

    assert(0 <= src && src < npusCount);
    assert(0 <= dest && dest < npusCount);

    return linkStartTimes[src][dest];
}

void TacosNetwork::setLinkStartTime(const LinkId link, const Time time) noexcept {
    const auto [src, dest] = link;

    assert(0 <= src && src < npusCount);
    assert(0 <= dest && dest < npusCount);

    linkStartTimes[src][dest] = time;
}
